...
export const Reports: React.FC<{ profile: UserProfile; theme: 'light' | 'dark' }> = ({ profile, theme }) => {
  ...
  const bgCard = theme === 'dark' ? 'bg-[#2a2a2a] border-[#3a3a3a]' : 'bg-white border-slate-200';
  ...
  const handleAIReport = async (reportName: string) => {
    ...
    // Filter for current year to match dashboard using robust string matching
    const currentYear = new Date().getFullYear();
    const currentYearIncomes = incomes.filter(i => {
      if (!i.date || typeof i.date !== 'string') return false;
      const parts = i.date.trim().split('-');
      if (parts.length === 0) return false;
      return parseInt(parts[0]) === currentYear;
    });

    const total = currentYearIncomes.reduce((acc, curr) => {
      const val = curr.amountUah ?? curr.amount; // Use consistent logic with Dashboard
      return acc + (val || 0);
    }, 0);
    const tax = total * profile.taxRate;

    const context = `
      Звітний рік: ${currentYear}
      Тип звіту: ${reportName}
      ПІБ ФОП: ${profile.name}
      Група: ${profile.group}
      Загальний дохід (за рік): ${total} UAH
      Податок до сплати (орієнтовно): ${tax} UAH
      Кількість операцій: ${currentYearIncomes.length}
    `;
  ...
  return (
    <div className={`p-4 md:p-8 space-y-6 min-h-screen ${theme === 'dark' ? 'bg-[#1f1f1f]' : 'bg-slate-50'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <button
          ...
          className={`flex items-center justify-between p-6 ${bgCard} border rounded-xl transition-all group text-left ${theme === 'dark'
            ? 'hover:border-blue-600 hover:bg-[#333333]'
            : 'hover:border-blue-500 hover:shadow-md'
            }`}
        >
          <div className="flex items-center gap-4">
            <div className={`p-3 rounded-lg transition-colors ${theme === 'dark'
              ? 'bg-[#3a3a3a] text-slate-300 group-hover:bg-blue-900/30 group-hover:text-blue-400'
        ...
        <button
          ...
          className={`flex items-center justify-between p-6 ${bgCard} border rounded-xl transition-all group text-left ${theme === 'dark'
            ? 'hover:border-emerald-600 hover:bg-[#333333]'
            : 'hover:border-emerald-500 hover:shadow-md'
            }`}
        >
          <div className="flex items-center gap-4">
            <div className={`p-3 rounded-lg transition-colors ${theme === 'dark'
              ? 'bg-[#3a3a3a] text-slate-300 group-hover:bg-emerald-900/30 group-hover:text-emerald-400'
      ...
      <div className={`rounded-xl p-4 text-sm border ${theme === 'dark'
